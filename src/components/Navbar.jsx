...
function Navbar() {
  ...
  return (
    <div data-scroll data-scroll-section data-scroll-speed= ".1" className='fixed z-[999] w-full px-5 py-4 flex justify-between items-center bg-zinc-900 shadow-md'>
      <div className="logo">
      ...
      {/* Mobile Menu */}
      <div className={`md:hidden ${isOpen ? 'block' : 'hidden'} absolute top-16 left-0 w-full bg-zinc-900 shadow-lg`}>
        {["Services", "Our Work", "About Us", "Insights", "Contact Us"].map((item, index) => (
          <a key={index} className="block px-4 py-2 text-md font-light border-b" onClick={toggleMenu}>{item}</a>
        ))}
      </div>
    </div>
  );
